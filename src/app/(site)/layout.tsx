...
import { CONFIGURACAO, RODAPE_SAIDAS } from '@/sanity/queries';
import type { CONFIGURACAO_RESULT, RODAPE_SAIDAS_RESULT } from '@/sanity/types';
...
export default async function LayoutSite({ children }: { children: React.ReactNode }) {
  const [config, proximas] = await Promise.all([
    ...
    buscar<RODAPE_SAIDAS_RESULT>(RODAPE_SAIDAS, {}, ['saida']),
  ]);
  ...
  return (
    <>
      ...
      <Rodape
        proximasSaidas={proximas.filter(
          (s): s is typeof s & { titulo: string; slug: string; dataInicio: string } =>
