# 10 — Dados iniciais e script de seed

Conteúdo fictício para construir e demonstrar o site antes de ter o material real do Felipe.

---

## 1. Regra de ouro

**Todo documento de seed tem `_id` com prefixo `seed.`**

Isso permite apagar tudo em um comando quando o conteúdo real chegar:

```groq
*[_id in path("seed.**")]
```

Sem esse prefixo, separar o fictício do real vira trabalho manual e arriscado — e alguém acaba publicando "Maria da Silva, Campinas" como depoimento verdadeiro.

**Segunda regra:** todo texto fictício começa com `[EXEMPLO]` nos campos que aparecem na tela durante a construção. Fica evidente na demonstração para o Felipe que aquilo é placeholder, e evita mal-entendido.

---

## 2. Imagens de placeholder

`picsum.photos` com seed determinístico — a mesma URL devolve sempre a mesma foto, então o layout não muda a cada seed.

```
https://picsum.photos/seed/{identificador}/1600/1067
```

O script baixa e sobe cada imagem como asset do Sanity, para o site funcionar exatamente como funcionará com as fotos reais (CDN, `lqip`, hotspot).

**Aviso a registrar no projeto:** essas imagens são placeholder de desenvolvimento. Nenhuma vai para o ar na entrega final. Antes de publicar com domínio próprio, rodar `npm run seed:limpar` e subir o acervo real.

---

## 3. Ramais

Dados aproximados, a conferir com o Felipe. Marcados como exemplo.

| Nome | Cidade de início | km (aprox.) | Dias | Dificuldade |
|---|---|---|---|---|
| Ramal Principal — Águas da Prata | Águas da Prata — SP | 415 | 13 | moderado |
| Ramal de Tambaú | Tambaú — SP | 320 | 11 | moderado |
| Ramal da Luminosa | Águas da Prata — SP | 430 | 14 | exigente |
| Ramal de Paraisópolis | Paraisópolis — MG | 240 | 8 | leve |
| Ramal de Estiva | Estiva — MG | 280 | 9 | moderado |

⚠️ **Conferir todos os números com o Felipe antes da entrega.** Distância errada num site de guia é erro que custa confiança.

---

## 4. Saídas fictícias

Três estados diferentes, para exercitar toda a lógica de status.

| Saída | Período | Ramal | Vagas | Estado exercitado |
|---|---|---|---|---|
| Saída de Setembro | 04–16/09/2026 | Águas da Prata | 4 de 14 | **Últimas vagas** |
| Saída de Novembro | 18–23/11/2026 | Paraisópolis | 12 de 12 | **Vagas abertas** |
| Saída de Estiva | 09–16/09/2026 | Estiva | 0 de 10 | **Esgotada** |
| Saída de Maio | 10–22/05/2026 | Águas da Prata | 0 de 12 | **Realizada** (data passada) |

A quarta existe para validar que saída passada continua no ar como prova social, com galeria e depoimentos, e sai da lista de próximas sozinha.
...
```json
{
  "scripts": {
    ...
    "seed:limpar": "tsx scripts/limpar-seed.ts"
  },
  "devDependencies": {
    "tsx": "^4.19.0",
    "dotenv": "^16.4.0"
...
## 6. Checklist de troca do fictício pelo real

Antes de apontar o domínio definitivo:

- [ ] `npm run seed:limpar`
- [ ] Nenhuma ocorrência de `[EXEMPLO]` em todo o dataset
- [ ] Nenhuma imagem vinda do picsum
- [ ] Distâncias e ramais conferidos com o Felipe
- [ ] Texto do "Quem sou" escrito por ele
- [ ] Depoimentos reais, todos com autorização de uso de nome e cidade
- [ ] Valores reais das saídas
- [ ] E-mail do Studio trocado para o do Felipe, com ele como Administrator
- [ ] WhatsApp trocado para o dele em **Configurações do site**
- [ ] PDF real do material de captura carregado
- [ ] Domínio registrado no CPF dele e apontado
- [ ] Search Console com ele como proprietário

Enquanto qualquer item estiver aberto, o site fica em URL de preview — não em domínio próprio, e não indexado.
